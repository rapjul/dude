...
def test_full_flow(
    ...
    scraper_application.run(urls=[file_url], pages=2, format="custom", parser="playwright", follow_urls=True)
...
def test_async_save(
    ...
    scraper_application.run(urls=[file_url], pages=2, format="custom", parser="playwright")
    mock_save.assert_called_with(expected_browser_data, None)


def test_format_not_supported(scraper_application: Scraper, async_playwright_select: None, file_url: str) -> None:
    assert scraper_application.has_async is True
    assert len(scraper_application.rules) == 4
...
def test_failed_to_save(
    scraper_application: Scraper, async_playwright_select: None, expected_browser_data: List[Dict], file_url: str
) -> None:
    assert scraper_application.has_async is True
    assert len(scraper_application.rules) == 4
